...
class SpaceInvaders():
    ...
    def get_state(self):
        ...
        """
        distance_joueur = int(math.sqrt(abs((self.get_player_X()-self.get_indavers_X()[0])*2 + (self.get_player_Y()-self.get_indavers_Y()[0])*2))) #obligé de caster en int car sinon erreur dans le réseau de neuronne
        sens_invader = int(math.sqrt(abs((self.get_indavers_X()[0]-self.prev_alien_x)*2 + (self.get_indavers_Y()[0]-self.prev_alien_y)*2)))
        distance_balle = int(math.sqrt(abs((self.get_bullet_X()-self.get_indavers_X()[0])*2 + (self.get_bullet_Y()-self.get_indavers_Y()[0])*2)))
