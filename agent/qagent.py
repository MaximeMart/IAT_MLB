...
class QAgent(AgentInterface):
    ...
    def __init__(self, spaceInvader: SpaceInvaders, eps_profile: EpsilonProfile, gamma: float, alpha: float):
        ...
        self.Q = np.zeros([maze.ny, maze.nx, maze.na]) #TO MODIFY

        self.spaceInvader = spaceInvader
        self.na = maze.na #TO MODIFY
        ...
        self.values = pd.DataFrame(data={'nx': [maze.nx], 'ny': [maze.ny]}) #TO MODIFY
    ...
    def learn(self, env, n_episodes, max_steps):
        ...
        for episode in range(n_episodes):
            ...
            state = env.reset_using_existing_maze() #TO MODIFY
            # Execute K steps 
    ...
    def save_log(self, env, episode):
        ...
        V = np.zeros((int(self.maze.ny), int(self.maze.nx))) #TO MODIFY
        for state in self.maze.getStates(): #TO MODIFY
        ...
        self.values = self.values.append({'episode': episode, 'value': np.reshape(V,(1, self.maze.ny*self.maze.nx))[0]},ignore_index=True) #TO MODIFY